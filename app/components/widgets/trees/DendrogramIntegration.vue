<!-- components/widgets/backgrounds/trees/DendrogramIntegration.vue -->
<script setup>
...
// Ищем zoomGroup в VueFlow
const findZoomGroup = () => {
  if (!containerRef.value) return null
  const svg = containerRef.value.closest('.vue-flow')?.querySelector('.vue-flow__viewport')
  return svg
}
...
const renderGraph = () => {
  ...
  const container = containerRef.value.closest('.tree-canvas-wrapper')
  if (!container) return
...
watchEffect(() => {
  if (containerRef.value) {
    ...
    if (zoomGroup) {
      ...
      observer.observe(containerRef.value.closest('.tree-canvas-wrapper') || document.body, {
        childList: true,
        subtree: true
      })
    }
